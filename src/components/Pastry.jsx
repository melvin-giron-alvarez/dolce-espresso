...
import { pastryIMG } from '../constants';
...
const Pastry = () => {
  ...
  return (
    <div id='pastry' className='w-[90%] mx-auto pt-[5em] md:pt-[6em] pb-[5em]'>
        ...
        className='w-full h-[515px] py-[3.5em] my-auto'>
        {pastryIMG.map((item, index) => (
            ...
            <p className={`relative text-center text-white leading-none ${index === activeIndex ? 'bottom-[40px] text-[1.5em] md:text-[1.9em] lg:text-[2.4em]' : 'text-[1em] md:text-[1.2em] lg:text-[1.4em] pt-[2%]' }`}>{item.text}</p>
            </SwiperSlide>
        ))}
        </Swiper>
            ...
            {pastryIMG.map((item, index) => (
                <div data-aos='fade-up' key={item.id} className={`sm:w-[45%] py-[1.5em]`}>
                    <div className='flex items-center justify-between'>
                        <p className='alice-regular text-[1.5em] pb-[.1em] leading-none lg:text-[1.8em]'>{item.text}</p>
                        <p className='text-[.89em] leading-tight md:text-[.9em] lg:text-[.95em]'>{item.price}</p>
                    </div>
